...
func (a *AccountDB) FindByID(id string) (*entity.Account, error) {
	...
	err = stmt.QueryRow(id).Scan(&account.ID, &account.Client.ID, &account.Balance, &account.CreatedAt, &account.Client.ID, &account.Client.Name, &account.Client.Email, &account.Client.CreatedAt)
